...
fn decl_cpp_cfg(encoding: &str) {
    let define = map_cpp_feature_define(encoding);
    g!("#if {define}");
}

fn decl_cpp_cfg2(from: &str, to: &str) {
    let from_define = map_cpp_feature_define(from);
    let to_define = map_cpp_feature_define(to);
    if from_define == to_define {
        g!("#if {from_define}");
    } else {
        g!("#if {from_define} && {to_define}");
    }
}

fn decl_cpp_endif(encoding: &str) {
    let define = map_cpp_feature_define(encoding);
    g!("#endif // {define}");
}

fn decl_cpp_endif2(from: &str, to: &str) {
    let from_define = map_cpp_feature_define(from);
    let to_define = map_cpp_feature_define(to);
    if from_define == to_define {
        g!("#endif // {from_define}");
    } else {
        g!("#endif // {from_define} && {to_define}");
    }
}
...
pub fn codegen_cpp() {
    ...
    g!("}}");
    g!("#endif // SIMDUTF_FEATURE_BASE64");
    g!();
...
pub fn codegen_rust() {
    ...
    for_each_validate(|encoding| {
        ...
        let feature = map_feature(encoding);
        g!(r#"#[cfg(feature = "{feature}")]"#);
    ...
    for_each_transcoding_length(|from, to| {
        ...
        let from_feature = map_feature(from);
        let to_feature = map_feature(to);
        if from_feature == to_feature {
            g!(r#"#[cfg(feature = "{from_feature}")]"#);
        } else {
            g!(r#"#[cfg(all(feature = "{from_feature}", feature = "{to_feature}"))]"#);
        }
        if is_fixed_length_for_latin1(from, to) {
