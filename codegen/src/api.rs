...
fn codegen_validate(g: &mut Codegen) {
    ...
    for_each_validate(|encoding| {
        ...
        g.ln("#[inline]");
        g.ln("#[must_use]");
        g.ln(f!("pub fn validate_{encoding}_with_errors(src: &[{ch}]) -> Result {{"));
        g.ln("let len = src.len();");
        g.ln("let buf = src.as_ptr();");
        g.ln(f!("unsafe {{ crate::bindings::simdutf_validate_{encoding}_with_errors(buf, len) }}"));
        g.ln("}");
        g.lf();
    });
}
...
fn codegen_transcoding_convert(g: &mut Codegen) {
    ...
    for_each_transcoding_convert(|from, to| {
        ...
        g.ln("#[inline]");
        g.ln("#[must_use]");
        g.ln(f!("pub unsafe fn convert_{from}_to_{to}_with_errors\
            (src: *const {from_ch}, len: usize, dst: *mut {to_ch}) -> Result {{"));
        g.ln(f!("crate::bindings::simdutf_convert_{from}_to_{to}_with_errors(src, len, dst)"));
        g.ln("}");
        g.lf();
    });
